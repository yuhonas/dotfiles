...
class ElmAnalyse {
        ...
        try {
            ...
            this.analyseSocket.on('message', stateJson => {
                try {
                    ...
                    var failedMessages = messages.map(message => this.parseMessage(cwd, message)).filter(result => !result.success);
                    if (failedMessages.length > 0) {
                        ...
                        vscode.window.showErrorMessage(messageText, 'Show details').then(item => {
    ...
    parseMessageInfoFileRanges(messageInfoData) {
        var messageInfoFileRanges;
        var messageInfoProperties = messageInfoData.properties;
    ...
    correctRangeWithinDescription(originalRegions, originalDescription) {
        function formatRegion(region) {
            return `((${region.start.line},${region.start.column}),(${region.end.line},${region.end.column}))`;
        }
        return originalRegions.reduce((currentDescription, originalRegion) => {
