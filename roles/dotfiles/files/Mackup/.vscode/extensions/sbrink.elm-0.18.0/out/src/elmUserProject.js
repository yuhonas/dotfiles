...
function userProject(document, position, currentWord, action) {
    ...
    let imports = [];
    let results = [];
    const cwd = elmUtils_1.detectProjectRoot(document.fileName) || vscode.workspace.rootPath;
    gCwd = cwd;
    const elmPackageString = fs.readFileSync(path.join(cwd, 'elm-package.json'), 'utf-8');
